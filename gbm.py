...
class GradientBoostingCustom(BaseEstimator):
    def __init__(self, loss, use_custom_tree=False, n_estimators=10, learning_rate=1e-1, max_depth=3):
        ...
        if use_custom_tree:
            self.tree_class = partial(DecisionTree, criterion='variance', max_depth=max_depth)
        else:
            self.tree_class = partial(DecisionTreeRegressor, max_depth=max_depth)

        if loss == 'mse':
            self.loss = mean_squared_error
            self.grad = self._mse_grad

        elif loss == 'rmsle':
            self.loss = self.rmsle
            self.grad = self._rmsle_grad

        elif loss == 'log_loss':
            self.loss = self._log_loss
            self.grad = self._log_loss_grad

        elif loss == 'poisson':
            self.loss = self._poisson
            self.grad = self._grad_possion
